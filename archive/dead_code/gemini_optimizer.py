...
Generate a NEW title matching this length. Count carefully: 75-80 characters!"""
    ...
    def _process_aspects(self, specs_list: List[Dict], is_oversize: bool) -> Dict[str, List[str]]:
        ...
        # Auto-detect and add required aspects for specific categories
        type_value = aspects.get("Type", [""])[0].lower() if aspects.get("Type") else ""
        material_value = aspects.get("Material", [""])[0].lower() if aspects.get("Material") else ""
        
        # For Sofas - Upholstery Fabric is required
        if type_value in ["sofa", "sectional", "couch", "loveseat"]:
            if "Upholstery Fabric" not in aspects:
                # Try to detect fabric from Material
                if "corduroy" in material_value:
                    aspects["Upholstery Fabric"] = ["Corduroy"]
                elif "velvet" in material_value:
                    aspects["Upholstery Fabric"] = ["Velvet"]
                elif "linen" in material_value:
                    aspects["Upholstery Fabric"] = ["Linen"]
                elif "leather" in material_value:
                    aspects["Upholstery Fabric"] = ["Faux Leather"]
                elif "fabric" in material_value:
                    aspects["Upholstery Fabric"] = ["Polyester"]
                else:
                    aspects["Upholstery Fabric"] = ["Fabric"]
