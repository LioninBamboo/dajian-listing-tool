...
def test_create_inventory_item_preserves_combined_source_material_and_color():
    client = object.__new__(RealEbayClient)
    client.base_url = "https://api.ebay.example"
    client.oauth = _DummyOauth()
    client.session = _Session()
    ...
    assert result["status"] == "success"
    assert client.session.last_json["product"]["aspects"]["Material"] == ["MDF,Rubber Wood"]
    assert client.session.last_json["product"]["aspects"]["Color"] == ["Black,Espresso"]
